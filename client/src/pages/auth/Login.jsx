...
export default function Login() {
  ...
  const demoUsers = [
    ['Manager Demo', 'manager@demo.com'],
    ['Agent 2 (Billing)', 'agent2@demo.com'],
    ['Agent 3 (Tech)', 'agent3@demo.com'],
  ];
        ...
        <div className="w-full max-w-md rounded-2xl border border-emerald-100 bg-white/90 p-7 shadow-xl shadow-emerald-900/5 backdrop-blur-sm sm:p-10">
          ...
          <div className="mt-8 border-t border-emerald-800/60 pt-6">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {demoUsers.map(([label, email]) => <button key={email} type="button" onClick={() => setValue('email', email, { shouldValidate: true })} className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] font-semibold text-emerald-700 transition hover:border-emerald-400 hover:bg-emerald-100">{label}</button>)}
            </div>
          </div>
