...
export default function ManagerDashboard() {
  ...
  const loadUnassignedTickets = useCallback(async (showLoader = true, agentsList = null) => {
    ...
    try {
      const response = await API.get('/manager/unassigned-tickets/?scope=all');
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="flex flex-col gap-4 rounded-[18px] border border-[#dfe5e1] bg-white px-5 py-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-[24px] font-bold tracking-tight text-[#1c2430]">All Tickets & Assignment Queue</h1>
          <p className="mt-1 text-[12px] text-[#64748b]">Review ownership, routing reason, and RAG diagnosis before assigning or reassigning tickets.</p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row">
          <label className="relative min-w-[280px]">
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search by ticket, customer, category, or priority"
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon={FiInbox} label="Unassigned Tickets" value={metrics.totalUnassigned} helper="Awaiting agent assignment" tone="amber" />
      ...
      <div className="mt-6 rounded-[14px] border border-[#dfe5e1] bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-[#dfe5e1] px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-[15px] font-bold text-[#1c2430]">Unassigned Queue</h2>
            <div className="mt-1 text-[11.5px] text-[#64748b]">Tickets are sorted by priority then AI diagnosis confidence.</div>
          </div>
        ...
        <div className="flex items-center justify-between border-t border-[#dfe5e1] px-4 py-3 text-[11.5px] text-[#64748b]">
          <span>Showing {filteredTickets.length} of {tickets.length} unassigned tickets</span>
