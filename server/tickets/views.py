...
class TicketViewSet(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            user_email = getattr(request.user, 'email', 'user@example.com')
            subject = (
...
def get_customer_email_logs(request):
    ...
    for log in logs:
        ...
        subject_label = subject_labels.get(log.email_type, log.email_type.replace("_", " ").title())
        subject = f"[SupportPilot] {subject_label}: {ticket_title} (#{ticket_id[-8:]})"
